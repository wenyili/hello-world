	...
	if (pathmax == 0) { /* first time through */
		...
		if ((pathmax = pathconf("/", _PC_PATH_MAX)) < 0) {
			if (errno == 0)
				...
				err_sys("pathconf error for _PC_PATH_PAX");
		} else {
			pathmax++;	/* add one since it's relative */
		}
	}
	/*
	 * Befor POSIX.1-2001, we aren't guaranteed that PATH_MAX include
	 * the teminating null byte. Same goes for XPG3.
	 */
